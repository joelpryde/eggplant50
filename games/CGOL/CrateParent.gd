...
onready var goal_prefab = load("res://games/CGOL/Fill.tscn")
...
onready var pop_player = get_parent().get_node("PopPlayer")
onready var win_player = get_parent().get_node("WinPlayer")
onready var player = get_parent().get_node("Player")
onready var goal_parent = get_parent().get_node("FillParent")
...
func create_goal(x, y):
	...
	if goals.has(position_key(x, y)):
		return
	goals[position_key(x, y)] = true
	var goal = goal_prefab.instance()
	goal.z_index = 1
	goal_parent.add_child(goal)
	goal.position = Vector2(x * constants.grid_cell_size, y * constants.grid_cell_size)
...
func clear_grid():
	...
	for fill in goal_parent.get_children():
		fill.queue_free()
	goals = {}
	for childCrate in get_children():
